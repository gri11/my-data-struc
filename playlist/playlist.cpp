  ...
  public:
    ...
    void reorder(int pos,std::vector<int> selected) {
      //write your code only here
			...
			iterator pit = begin();
			for(int i = 0; i < pos; pos++, pit++);

			// 1) insert, erase
			// int i = mSize-1, j = selected.size()-1;
			// for(iterator it = mHeader->prev; it != mHeader; it--, i--){
			// 	if(i == selected[j] && j >= 0) {
			// 		// std::cout << *pit << '\n';
			// 		pit = insert(pit, *it);
			// 		erase(it);
			...
			// 2) chain
			// if j == 0 : hanging chain
			// else : append chain
			// all select close chain
			// int i = 0, j = 0;
			// iterator cit;
      ...
      //     if(j == selected.size()-1){
			...
			// 			it.ptr->prev->next = it.ptr->next;
			// 			it.ptr->next->prev = it.ptr->prev;
			// 			pit.ptr->prev->next = it.ptr;
			// 			it.ptr->prev = pit.ptr->prev;
			// 			cit = it;
			// 		}
			...
			int i = 0, j = 0;
			for(iterator it = begin(); it != end(); it++, i++){
				if(i == selected[j] && j < selected.size()) {
					its.push_back(it);
          // erase(it);
					j++;
				}
